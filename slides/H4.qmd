...
* Een lineaire afbeelding $\map{A}\in \Hom(V,W)$ wordt begrends genoemd als er een constante $C \in \bbR_{\geq 0}$ bestaat zodat
...
* voor een matrix $\mat{A}\in\bbF^{m \times n}$ kunnen we in principe de geïnduceerde norm berekenen voor $p$-norm op $\bbF^m$ en $q$-norm op $\bbF^n$; typisch willen we $p=q$.
...
* de normen $\norm{\mat{A}}_{p\times p}$ is consistent (bewijs aan bord)
* de norm $\norm{\mat{A}}_{\text{F}}=\norm{\text{vectorize}(\mat{A})}_2$ is consistent (bewijs aan bord)
...
* De consistentievoorwaarde is een veralgemening van submultiplicativiteit, en beide termen worden soms door elkaar gebruikt.
