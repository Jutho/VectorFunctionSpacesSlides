...
* Aan elke $\vv\in V$ associeren we een $\chi_\vv \in V^\ast$
  ...
  * Lineaire functionaal $\chi_\vv$ is continue en dus ook begrensd
  * Inderdaad: $\abs{\chi_\vv[w]} \leq \norm{\vv} \norm{ww}$ (Cauchy-Schwarz)
...
* $\chi_\vv[w] = \inner{\vv}{\vw} = \overline{v^i} g_{ij} w^j = (\chi_\vv)_j w^j$
  ...
  $\Rightarrow \chi_\vv = \overline{v^i} g_{ij} \epsilon^j$

* Inverse metriek: $\mat{g}^{-1} \mat{g} = \idmat \iff g^{ij} g_{jk} = \delta^i_{\ k}$
...
* $V$ en $V^\ast$ zijn (anti)isomorf als Hilbertruimten:
  ...
  de bijectieve afbeelding $\vv \to \chi_\vv$ behoudt inwendig product en lineaire combinaties (meer met complexe toevoeging van scalaire factoren)
...
* $\ker(\map{A})$ is altijd gesloten deelruimte
...
* Basis voor $W$: $B_W = \{\vf_1,\ldots,\vf_m\}$
  ...
  $\Rightarrow$ Duale basis voor $W^\ast$: $B_W^\ast = \{\varphi^m1,\ldots,\varphi^m\}$
...
* Als $B_W$ een orthonormale basis is: $A^i_{\ j}= \inner{\vf_i}{\map{A}\ve_j}$
  ...
  * Inderdaad: $\varphi^i[\vf_j] = \delta^i_{\ j} = \inner{\vf_i}{\vf_j}$
    ...
    $\Rightarrow$ $\vf_i$ is de vector geassocieerd aan de lineaire basisfunctionaal $\varphi^i$
