...
* Intuitie krijgen voor de spectraaltheorie van begrensde en onbegrensde operatoren op oneindig-dimensionale Hilbertruimten aan de hand van relevante voorbeelden

# Functieruimten en normen

(geen stellingen of bewijzen)

## Functieruimten {.smaller}

Gegeven een vectorruimte $Y$ over $\bbF$ en een willekeurige verzameling $X$:

verzameling $X^Y$ van functies $f:Y \to X$ vormt een vectorruimte over $\bbF$, waarbij de lineaire structuur gewoon puntsgewijs wordt gedefinieerd m.b.v. deze van $Y$:
...
* preciese definitie hangt af van integraalconcept
...
## Van seminorm naar norm {.smaller}
...
* functies waarvoor $\norm{f - g}_{p} = 0 \Leftrightarrow f-g \in U$ noemen we "bijna overal" gelijk zijn; dit is een equivalentierelatie
...
## "Bijna overal" en "maat nul" (nulverzamelingen) {.smaller}
...
## Compleetheid van $L^p(I;\bbF)$ {.smaller}

Ten opzichte van de Lebesgue-integraal zijn er stellingen die garanderen dat, voor een rij $(f_n)$ die "bijna overal" puntsgewijs convergeert naar $f$, 

* $\lim_{n\to \infty} \int_I f_n(x)$\,\drm x = \int_I \lim_{n\to\infty} f_n(x)\,\drm x$ als

  $\forall n \in \bbN_0$: $f_n(x) \leq f_{n+1}(x)$ "bijna overal"

  of

  $\forall n \in \bbN_0$: $\abs{f_n(x)} \leq g(x)$ "bijna overal" voor een integreerbare functie $g$

* deze resultaten kunnen worden gebruikt om aan te tonen dat

  $L^p(I,\bbF)$ is Banachruimte (stelling van Riesz-Fischer)
...
* $L^p(I,\bbF)$ is "separabel": laat aftelbare fundamentele verzameling $S$ toe
  ...
  * $I$ compact: $S=\{x^k,k\in\bbN\}$, $S=\{\exp(\irm \frac{2\pi}{L} x), k \in\bbZ\}$, \dots
  * $I = \bbR$: $S=\{x^n \chi_{[-N,N]}, n \in \bbN, N \in \bbN\}$

# Hilbertruimten en inwendig product

## Hilbertruimten en inwendig product {.smaller}
...
## Orthogonale veeltermen {.smaller}
...
* $x p_n(x) = a_{n} p_{n+1}(x) + b_n p_{n}(x) + c_{n} p_{n-1}(x)$ (bewijs aan bord)
...
## Orthogonale veeltermen: Legendre {.smaller}
...
## Orthogonale veeltermen: Hermite {.smaller}
...
## Orthogonale veeltermen: Laguerre {.smaller}
...
## Orthogonale veeltermen: Chebyshev {.smaller}
...
## Toepassing: Gaussische kwadratuur {.smaller}
...
## Trigonometrische veeltermen en Fouriercoefficiënten {.smaller}

* **Fouriercoefficiënten**: $\widehat{f}_k = \inner{\varphi_k}{f} = \frac{1}{\sqrt{L}} \int_{0}^{L} f(x) \erm^{-\irm \frac{2\pi}{L} x}\,\drm x$ ($\forall k \in \bbZ$)
...
* Orthogonale projector op trigonometrische veeltermen van graad $n$:
  ...
  \begin{align}
    ...
    &= \sum_{k=-n}^{+n} \frac{1}{L} \int_0^L f(t) \erm^{+ \irm \frac{2\pi}{L} k (x - t)}\,\drm t = \int_0^L D_n(x-t) f(t)\,\drm t\label{eq:function:fourierapproximationn}
  \end{align}
...
## Fouriercoefficiënten: eigenschappen {.smaller}

$$\widehat{f}_k = \inner{\varphi_k}{f} = \frac{1}{\sqrt{L}} \int_{0}^{L} f(x) \erm^{-\irm \frac{2\pi}{L} x}\,\drm x$$

 * Bestaan ($\forall k \in \bbZ$) voor elke $f \in L^1([0,L];\bbC)$ met $\abs{\widehat{f}_k} \leq \frac{1}{\sqrt{L}} \norm{f}_1 \Rightarrow \norm{\widehat{f}}_{\infty} =  \sup_{k \in \bbZ} \abs{\widehat{f}}_{\infty} \leq \frac{1}{\sqrt{L}}\norm{f}_1$
...
## Convergentie (geen stellingen/bewijzen){.smaller}
...
## Gladheid en convergentiesnelheid {.smaller}

* Een periodieke functie is continu, i.e. $f \in C^(\bbT^1_L)$, als
  	\begin{align*}
...
## Convoluties herbekeken {.smaller}
...
## Stuksgewijze continuïteit en Gibbsfenoneem {.smaller}
...
## Relatie met discrete Fouriertransformatie {.smaller}
...
* Fouriercoefficiënten exact begrensd: $\widehat{f}_k = 0$ voor $k>n$ $\Leftrightarrow$ $f$ is trigonometrische veelterm van graad $n$

  $\Rightarrow$ exacte reconstructie mogelijk uit samples als $\epsilon \leq L/(2n+1)$

# Operatoren op Hilbertruimten

## Veelvoorkomende operatoren op functieruimten {.smaller}
...
## Begrensde en onbegrensde operatoren {.smaller}
...
* Afleidingsoperator: altijd onbegrensd
  ...
  * op $L^2([0,L],\bbC)$: $f_k = \frac{1}{\sqrt{L}} \exp(\irm \frac{2\pi}{k} x)$
...
* $[\map{A},\map{B}] = \idmap$ (of technisch: restrictie $\subset \idmap$):
  ...
  $\Rightarrow$ minstens 1 van beide operatoren is onbegrensd (bewijs aan bord)
...
* Voorbeeld 2: $H = \ell^2(\bbN)$: rijen $\vv=(v_0,v_1,v_2,\ldots)$ met $\sum_{n \in \bbN} \abs{\vv_n}^2 < \infty$
  ...
  * \begin{align}
    ...
    \sqrt{n} \ve_{n-1},&n > 0
    \end{cases}, &\map{A}^+\ve_{n} = \sqrt{n+1} \ve_{n+1}.\label{eq:function:annihilationcreation}
    \end{align}
...
## Toegevoegde van een onbegrensde operator {.smaller}
...
## Toegevoegde: eigenschappen en voorbeelden {.smaller}
...
* Voorbeeld 2: op $L^2([a,b],\bbF)$, beschouw $\map{D}$ met als domein alle afleidbare functies met $f' \in L^2([a,b],\bbF)$ en $f(a)=f(b)=0$
  ...
  $\Rightarrow$ $(\map{D}^\dagger g)(x) = -g'(x)$ met als domein alle afleidbare functies $g$ met $f' \in L^2([a,b],\bbF)$, zonder bijkomende randvoorwaarden
...
* Als $\map{A}^\dagger$ zelf ook dicht gedefineerd is, dan kunnen we $\map{A}^{\dagger\dagger}$ definiëren:
  ...
  $\rightarrow$ $\map{A} \subset \map{A}^{\dagger\dagger}$ ($\D_{\map{A}^{\dagger\dagger}}$ bevat alle $\vv$ zodat $\vw \mapsto \inner{\vv}{\map{A}^\dagger\vw} = \conj{\inner{\map{A}^\dagger \vw}{\vv}}$ continu is: zeker voldaan $\vv \in \D_{\map{A}}$)

## Symmetrische en zelftoegevoegde operatoren {.smaller}

* **symmetrische / hermitische / hermitisch symmetrische operator**:
  $$\inner{\vv}{\map{A}\vv} = \inner{\map{A}\vv}{\vw}, \forall \vv,\vw \in \D\,\!_{\map{A}}$$
...
## Positie- en momentumoperator op $L^2(\bbR)$ {.smaller}
...
## Eigenwaarden en eigenvectoren {.smaller}
...
## Compacte operatoren (geen stellingen/bewijzen){.smaller}
...
## Spectrum van algemene operatoren {.smaller}

* Operator $\map{A}$ met dicht domein $\D_{\map{A}}$ en bereik $\mathcal{R}_{\map{A}}$ is injectieef indien $\ker(\map{A})=\{\zerovec\}$
...
## Spectrum van algemene operatoren: voorbeelden {.smaller}
...
## Spectrum van zelftoegevoegde operatoren {.smaller}
