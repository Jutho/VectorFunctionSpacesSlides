...
* $\varphi: A \to B : a \mapsto \varphi(a)$:
  ...
  - invers beeld (preimage) van $D \subseteq B$:^[Invers beeld gedraagt vereist geen injectiviteit, en gedraagt zich beter dan beeld onder unie en doorsnede: $\varphi^{-1}(D_1 \cup D_2) = \varphi^{-1}(D_1) \cup \varphi^{-1}(D_2)$ en analoog voor $\cap$]
...
* Werken met oneindige cardinaliteit:
  ...
  $\vert A \vert \leq \vert B \vert \Leftrightarrow$ er bestaat een injectieve afbeelding van $B$ naar $A$
...
In een ring is er een neutraal element $0$ van $+$ en een (ander!) neutraal element $1$ van $\cdot$. Verder kunnen er nog elementen zijn met een speciale rol:
...
* $\bbF^{n\times m}$: $n \times m$ matrices, opnieuw met componentsgewijze optelling en vermenigvuldiging met scalairen (≠ matrixvermenigvuldiging)
...
* Voor $\mathcal{D} \subseteq \bbR^n$: Optelling en scalaire vermenigvuldiging behoudt de meeste analytische eigenschappen, dus interessante deelruimtes:
  ...
  - $n$-maal continue afleidbare functies: $C^k(\mathcal{D},\bbF)$
  - gladde functies: $C^{\infty}(\mathcal{D},\bbF)$
  - analytische functies $C^{\omega}(\mathcal{D},\bbF)$
...
* $\vec{v} = \phi_B(\vv)$: $\phi_B$ coordinatenisomorfisme
...
* als $W_1 \cap W_2 = \{\zerovec\}$: **(interne) directe som** $W_1 \oplus W_2$
  * ontbinding $\vv = \vw_1 + \vw_2$ is uniek
...
* Vectorstructuur:
  * $(\vv_1,\vv_2) + (\vw_1, \vw_2) = (\vw_1,\vw_2)$
  * $a (\vv_1, \vv_2)= (a \vv_1, a \vv_2)$
* notatie: $V_1 \oplus V_2$, naam: **(externe) directe som**
* $\vv_1 \mapsto (\vv_1,\zerovec_{V_2})$ en $\vv_2 \mapsto (\zerovec_{V_1},\vv_2)$ definiëren natuurlijke inbedding van $V_1$ en $V_2$ als subruimtes van $V_1 \oplus V_2$
...
* **quotientruimte** $V/W$:
  ...
  * equivalentieklassen: $\vv+ W = \{\vv + \vw; \vw \vw \in W\}$
  * $\dim(V/W) = \dim(V) - \dim(W)$

## Affiene ruimte

Gegeven een vectorruimte $V$, een geassocierde affiene ruimte is een verzameling van punten $A = \{P, Q, \ldots\}$ waarop de abelse groep $(V,+)$ een groepsactie heeft, of dus
...
* $\forall P, Q \in A$: er bestaat een $\vv$ zodat $Q = P + \vv$
  * notatie: $\vv = Q - P$
...
dat lineair is beide argumenten afzonderlijk (= bilineair)
...
* Lie-algebra: algebra met alternered product, genoteerd als $[\vv, \vw] = -[\vw,\vv]$ dat voldoet aan Jacobi-identiteit
  ...
  $\rightarrow$ kan gemaakt worden uit associatieve algebra m.b.v. commutator
